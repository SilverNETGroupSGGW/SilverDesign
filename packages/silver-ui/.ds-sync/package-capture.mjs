...
try {
  ...
  console.error('playwright not installed — npm i playwright (in .ds-sync/) first');
  process.exit(2);
}
...
for (const c of comps) {
  ...
  for (const label of cells) {
    try {
      await page.goto(`http://127.0.0.1:${port}/${rel}?story=${encodeURIComponent(label)}`, {
        waitUntil: 'networkidle',
        timeout: 20_000,
      });
    } catch (e) {
      if (!/Timeout/i.test(String(e.message ?? e))) {
        shots.push({ label, png: null, err: String(e.message ?? e).split('\n')[0] });
        continue;
      }
    }
    await settle();
  ...
  const pendingGrade = !(
    cells.length > 0 && cells.every((k) => grade?.cells?.[k]?.verdict === 'good')
  );
  writeFileSync(
    capPath,
    JSON.stringify(
...
if (!ONLY) {
  ...
  // can't silently ship.
  try {
    const unmerged = readdirSync(resolve('.design-sync', 'learnings')).filter((f) =>
      f.endsWith('.md'),
    );
    if (unmerged.length)
      console.error(
