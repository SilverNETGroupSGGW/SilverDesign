...
try {
  ...
  if (!sync.styleSha || n < 0 || (n === 0 && ver?.componentCount !== 0))
    ...
    let live = null;
    try {
      live = createHash('sha256').update(readFileSync(bundleJs)).digest('hex').slice(0, 12);
    } catch {
  ...
  try {
    let manifest = null;
    try {
      manifest = JSON.parse(readFileSync(join(OUT, '.stories-map.json'), 'utf8'));
    } catch {
      ok(
    ...
    if (manifest) {
      ...
      if (stale.length)
        ...
        ok(`_ds_sync.json render hashes match disk (${manifest.components.length} recomputed)`);
    }
  } catch (e) {
    fail(`_ds_sync.json recompute failed (${String(e.message ?? e).split('\n')[0]})`);
  }
} catch (e) {
...
if (!existsSync(stylesCss)) fail('styles.css missing — the styles entry point the app reads');
  ...
  for (const m of txt.matchAll(/@import\s+(?:url\()?["']([^"']+)["']/g)) {
    ...
    if (!existsSync(join(OUT, m[1]))) {
      missing++;
      fail(`[CSS_IMPORT_MISSING] styles.css @imports "${m[1]}" which doesn't exist under ${OUT}`);
    }
  }
  if (n > 0) {
...
try {
  ...
  (function scanStyles(d) {
    ...
    for (const e of readdirSync(d, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) scanStyles(p);
        ...
        for (const m of readFileSync(p, 'utf8').matchAll(/<style\b[^>]*>([\s\S]*?)<\/style>/gi))
          allCss += '\n' + m[1];
      }
    }
  })(join(OUT, 'components'));
  ...
  const referenced = [
    ...new Set([...allCss.matchAll(/var\(\s*(--[\w-]+)\s*\)/g)].map((m) => m[1])),
  ];
...
try {
  ...
  const hasRemoteFonts =
    /@import[^;]*(?:fonts\.googleapis|fonts\.gstatic|use\.typekit|fonts\.bunny)/i.test(css);
  ...
  for (const e of readdirSync(d, { withFileTypes: true })) {
    ...
    if (e.name.endsWith('.html')) {
      ...
      if (!/^<!--\s*@dsCard\s+group="[^"]*"[^>]*-->/.test(txt.split('\n', 1)[0])) {
        badCard++;
        fail(`[DSCARD_MISSING] ${rel}: first line isn't a \`<!-- @dsCard group="…" -->\` comment`);
      }
      for (const m of txt.matchAll(/<link\b[^>]*\bhref="([^"]+)"/g)) {
        ...
        if (!existsSync(resolve(dirname(p), m[1]))) {
          badLink++;
          fail(`[LINK_HREF_MISSING] ${rel}: <link href="${m[1]}"> doesn't resolve`);
        }
      }
  ...
  try {
    ...
    if (headerMeta?.components?.length) {
      try {
        ...
        await page
          .waitForFunction((g) => !!window[g], headerMeta.namespace, { timeout: 10_000 })
          .catch(() => {});
    ...
    for (const rel of sample) {
      ...
      try {
        ...
        ({
          ...
          const portals = [...document.body.children].filter(
            (c) => !c.matches('#root, [id^="r"], .ds-grid, .ds-cell, section, script, style, link'),
          );
          let caught = 0,
            firstCaught = null,
            nEls = 0,
          ...
          if (window.__dsMode === 'grid' || window.__dsMode === 'column') {
            ...
            const subtreeShows = (root) => {
              ...
              for (const el of [root, ...root.querySelectorAll('*')]) {
                ...
                if (/^(IMG|SVG|CANVAS|VIDEO|IFRAME|PICTURE|HR)$/.test(el.tagName)) return true;
                if ([...el.childNodes].some((t) => t.nodeType === 3 && t.textContent.trim()))
                  return true;
            ...
            for (const cell of document.querySelectorAll('section.ds-cell')) {
              ...
              let kind =
                window.__dsMode === 'grid' && cell.scrollWidth > cell.clientWidth + 8
                  ? 'wide'
                  : null;
              ...
              if (kind)
                gridOverflow = kind === 'escape' || gridOverflow === 'escape' ? 'escape' : 'wide';
            }
            ...
            gridOverflowCells =
              gridOverflow === 'escape' ? escapeCells : gridOverflow === 'wide' ? wideCells : [];
          }
      ...
      if (gridOverflow) {
        const who = gridOverflowCells.length
          ...
          : '';
        const pick = storyExports.length ? `one of: ${storyExports.join(', ')}` : 'best export'; // templated inside <...> below — no own brackets
        warn(
          gridOverflow === 'escape'
    ...
    try {
      ...
      if (results.length) {
        ...
        for (let s = 0; s < sheetCount; s++) {
          ...
          await page.goto(`http://127.0.0.1:${port}/_screenshots/.contact-sheet-${s + 1}.html`, {
            waitUntil: 'networkidle',
            timeout: 15000,
          });
          await page.evaluate(() =>
