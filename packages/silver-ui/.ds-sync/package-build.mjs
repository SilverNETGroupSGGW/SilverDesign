...
const { emitBuildMeta, emitPerComponent, emitReadme, emitReviewPage, vendorReact } =
  await loadLib('emit');
...
// bundle (no _ds_bundle.js and no .ds-bundle marker). --out is user-supplied.
{
  ...
  const unsafe =
    [resolve('/'), resolve(process.env.HOME ?? '/nonexistent'), process.cwd(), durable].includes(
      outAbs,
    ) ||
...
if (explicitCss && existsSync(bundleCss)) {
  ...
  const cand = explicitCss
    ...
    : src.cssEntry
      ? [src.cssEntry]
      : ['build/esm/styles.css', 'dist/styles.css', 'dist/style.css', 'styles.css'].map((c) =>
          join(PKG_DIR, c),
        );
  for (const p of cand) {
    if (existsSync(p)) {
...
const fontRules = [
  ...
  ...(explicitCss
    ? extractFonts(explicitCss, dirname(explicitCss), { fontsOut, roots: PKG_DIR })
    : []),
];
...
if (!tokenFiles.length && src.tokensCss?.length) {
  ...
  if (tokenFiles.length)
    console.error(`  tokens: ${tokenFiles.length} file(s) from source shape default`);
}
  ...
  if (ep.startsWith('./') || ep.startsWith('../') || isAbsolute(ep)) {
    ...
    try {
      ...
      const scan = (file, depth) => {
        ...
        for (const m of src2.matchAll(/export\s*\*\s*as\s+([A-Za-z_$][\w$]*)\s*from/g)) {
...
let components =
  src.shape === 'storybook' ? src.components.filter((c) => exported.has(c.name)) : src.components;
...
// name to be treated as a root.
{
  ...
  if (parentOf.size) {
    ...
    const sample = [...byParent]
      .slice(0, 3)
      .map(([p, s]) => `${p}←${s.slice(0, 3).join(',')}${s.length > 3 ? ',…' : ''}`)
      .join('; ');
    console.error(
...
for (const c of components) {
  ...
  if (
    d.category &&
    (!c.group || c.group === 'general' || c.group === 'misc' || c.group === uniformNamed)
  ) {
    const g = d.category
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
...
if (SKIP_DTS) {
  ...
  writeFileSync(
    metaPath,
    JSON.stringify({ ...JSON.parse(readFileSync(metaPath, 'utf8')), dtsStubbed: true }, null, 2) +
      '\n',
  );
  console.error(
...
// after grading (a post-grade slim changes contracts and clears grades).
{
  ...
  for (const [p, sz] of big) {
    console.error(
      `! [FILE_TOO_LARGE] ${p} is ${(sz / 1024 / 1024).toFixed(1)} MB — the upload rejects files over ${MAX / 1024 / 1024} MB. Slim it NOW (before grading): heavy dev-only deps (syntax highlighters, icons-as-code) usually don't belong in a preview or decorator bundle.`,
    );
  }
}
