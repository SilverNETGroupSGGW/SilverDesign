...
if (!OUT) {
  console.error('usage: remote-diff.mjs --local <ds-bundle dir> [--remote <saved-sidecar.json>]');
  process.exit(2);
}
...
try {
  ...
  console.error(`✗ ${OUT}/_ds_sync.json unreadable (${e.message}) — run package-build.mjs first`);
  process.exit(1);
}
if (!validSidecar(local)) {
  console.error('✗ local _ds_sync.json malformed (styleSha/renderHashes/sourceHashes) — rebuild');
  process.exit(1);
}
...
try {
  ...
  console.error(`✗ ${OUT}/_ds_bundle.js unreadable — run package-build.mjs first`);
  process.exit(1);
}
...
if (remotePath) {
  ...
  try {
    ...
    console.error(`! remote sidecar unreadable (${e.message}) — treating as no anchor`);
  }
...
if (!remote) {
  ...
  const keysOk = (s) =>
    s.sourceKeys && typeof s.sourceKeys === 'object' && !Array.isArray(s.sourceKeys);
  const useSourceKeys =
    keysOk(local) &&
    keysOk(remote) &&
  ...
  out.upload.aux =
    remote.auxSha === undefined || local.auxSha === undefined || remote.auxSha !== local.auxSha;
  out.upload.any =
