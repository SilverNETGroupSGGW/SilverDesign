...
const comps = manifest.components.filter(
  (c) => c.stories.length && (!ONLY || ONLY.has(c.name) || SPOT_PICKS.includes(c.name)),
);
...
const oldGradeKeyFor = (c) =>
  gradeKeyFrom(renderHashFor(OUT, c, { stories: c.stories, srcSha: c.srcSha ?? null }));
...
for (const c of comps) {
  ...
  let grade = null;
  try {
    prevCapture = JSON.parse(readFileSync(join(cacheDir, `${c.name}.json`), 'utf8'));
  } catch {
    /* first capture */
  }
  try {
    grade = JSON.parse(readFileSync(join(cacheDir, `${c.name}.grade.json`), 'utf8'));
  } catch {
    /* ungraded */
  }
...
for (const name of SPOT_PICKS) {
  ...
  if (!p) {
    console.error(`(spot-check pick "${name}" has no stories in .stories-map.json — ignored)`);
    continue;
  }
  if (FORCE) continue;
  ...
  for (const c of comps) {
    ...
    try {
      ...
      if (/Timeout/i.test(String(e.message ?? e)))
        console.error(`  (networkidle timeout on ${c.name} — capturing after settle)`);
    ...
    async function storyShot(label) {
      try {
        await dsPage.goto(`http://127.0.0.1:${outPort}/${rel}?story=${encodeURIComponent(label)}`, {
          waitUntil: 'networkidle',
          timeout: 20_000,
        });
      } catch (e) {
    ...
    try {
      ...
      console.error(`  (sheet skipped for ${c.name} — ${String(e).split('\n')[0]})`);
    }
    ...
    const pendingGrade = !(
      gradable.length > 0 &&
      gradable.every((r) => ['match', 'close'].includes(grade?.stories?.[r.story]?.verdict))
    );
    const entry = {
      name: c.name,
      group: c.group,
...
const hard = report.filter(
  (r) =>
    !r.skipped &&
    (r.verdict === 'error' ||
      (r.counts && (r.counts.error || r.counts.unpaired || r.counts['sb-error']))),
);
if (!ONLY) {
  ...
  const withGrades = report.map((r) => {
    ...
    try {
      return {
        ...r,
        grades: JSON.parse(readFileSync(join(cacheDir, `${r.name}.grade.json`), 'utf8')),
      };
    } catch {
  ...
  try {
    const pendingLearnings = readdirSync(resolve('.design-sync', 'learnings')).filter((f) =>
      f.endsWith('.md'),
    );
